...
def extract_wrapper(file_and_dest: tuple[Path, Path]) -> None:
    ...
    extract_mfcc_from_file(file, destination)


def extract_mfcc_from_file(file: Path, output_dir: Path) -> None:
...
def extract_mfcc_features(dataset_dir: Path, output_dir: Path, name: str) -> None:
...
if __name__ == '__main__':
    ...
    AFEW_OUT = Path(params['afew_mfcc'])
    TESS_IN = Path(params['tess_wav'])
    TESS_OUT = Path(params['tess_mfcc'])

    extract_mfcc_features(AFEW_IN, AFEW_OUT, 'AFEW')
    extract_mfcc_features(TESS_IN, TESS_OUT, 'TESS')
